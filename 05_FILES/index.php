...
if($_POST){
    ...
    if(!empty($_FILES["photo"] ["name"])) {
        ...
        if($_FILES["photo"] ["name"]> $maxSize) {
          
            $msg .= "<div class=\"alert alert-danger\" role=\"alert\">
            La taille du fichier est limité a 1 MO
            </div>";
            
        }

        $listExtensions = ["png" , "jpg" , "jpeg"];
        $listExtension = strrchr($_FILES["photo"] ["name"] ,".");

        if(!in_array($fileExtension , $listExtensions)){
            $msg .= "<div class=\"alert alert-danger\" role=\"alert\">
            merci de charger que du png  , jpg ou jpeg
            </div>";
        }

        if(empty($msg)){
            $msg .= "<div class=\"alert alert-danger\" role=\"alert\">
            votre voiture a bien ete chargee en bdd
            </div>";
        }
    }
